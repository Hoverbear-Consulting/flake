# From https://github.com/NixOS/nixpkgs/blob/0fcee2222d74c1482f087d99cf786c552dd3fa78/pkgs/development/python-modules/torch/default.nix
{ stdenv, lib, fetchFromGitHub, fetchpatch, python310,
  ...
  # Native build inputs
  cmake, util-linux, linkFarm, symlinkJoin, which, removeReferencesTo,
...
in python310Packages.buildPythonPackage rec {
  pname = "torch-vulkan";
  ...
  preBuild = ''
    ...
    ${python310.interpreter} setup.py build --cmake-only
  ...
  buildInputs = with python310Packages; [ blas blas.provider pybind11 ]
  ...
  checkInputs = with python310Packages; [ hypothesis ninja psutil ];

  checkPhase = with lib.versions; with lib.strings; concatStringsSep " " [
    "runHook preCheck"
    cudaStubEnv
    "${python310.interpreter} test/run_test.py"
    "--exclude"
    (concatStringsSep " " [
  ...
  postInstall = ''
    find "$out/${python310.sitePackages}/torch/include" "$out/${python310.sitePackages}/torch/lib" -type f -exec remove-references-to -t ${stdenv.cc} '{}' +

    mkdir $dev
    cp -r $out/${python310.sitePackages}/torch/include $dev/include
    cp -r $out/${python310.sitePackages}/torch/share $dev/share
    ...
    mkdir $lib
    mv $out/${python310.sitePackages}/torch/lib $lib/lib
    ln -s $lib/lib $out/${python310.sitePackages}/torch/lib
  '';
